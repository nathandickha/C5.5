...
// Slope + flats begin AFTER the steps along the selected entry wall axis.
const originCoord = stepWallFrame.wallCoord + stepWallFrame.inwardSign * stepFootprintLen;
const axisLen = stepWallFrame.axis === "x" ? length : width;
const originX = stepWallFrame.axis === "x" && stepWallFrame.inwardSign > 0
  ? axisStartWallX + stepFootprintLen
  : axisStartWallX;

// Persist for downstream systems / debugging
group.userData.stepFootprintLen = stepFootprintLen;
group.userData.originX = originX;
group.userData.stepWall = stepWallFrame.wall;
  ...
  for (let i = 0; i < pos.count; i++) {
    const axisCoord = stepWallFrame.axis === "x" ? pos.getX(i) : pos.getY(i);
    let dx = stepWallFrame.inwardSign * (axisCoord - originCoord);
    if (dx < 0) dx = 0;
